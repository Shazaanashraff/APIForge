 ...
 *
 * <p>Boundary analysis covers: just-below-min, at-min, at-max, just-above-max, and typical middle
 * values. Used by the test case generator to create negative/edge-case tests.
 *
 * <p>JQwik's {@link Arbitraries} API generates random values within valid ranges, complementing the
 * deterministic boundary points.
 */
public class BoundaryValueProvider {

  /**
   * Returns boundary integers for a numeric schema: {@code [min-1, min, min+1, mid, max-1, max,
   * max+1]}.
   */
  ...
  /**
   * Returns boundary strings for a string schema: {@code ["", minLength-string, maxLength-string,
   * (maxLength+1)-string]}.
   */
  public List<String> stringBoundaries(JsonNode schema) {
    ...
    String randomValid =
        Arbitraries.strings().ofMinLength(minLength).ofMaxLength(maxLength).alpha().sample();
  ...
  /**
   * Returns boundary array sizes for an array schema: {@code [empty, minItems-1, minItems,
   * maxItems, maxItems+1]}.
   */
