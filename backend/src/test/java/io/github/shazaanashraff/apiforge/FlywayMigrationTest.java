...
/**
 * Verifies that all Flyway migrations run in order on a clean database.
 * Runs against plain Postgres (no TimescaleDB) — uses the test migration set.
 */
@Testcontainers
@SpringBootTest
@ActiveProfiles("test")
...
class FlywayMigrationTest {
  ...
  void allMigrationsApplySuccessfully() {
    ...
    long failedMigrations = java.util.Arrays.stream(info.applied())
        .filter(m -> m.getState().isFailed())
        .count();
    assertThat(failedMigrations)
        .as("No Flyway migrations should have failed")
        .isZero();
